...
if status == MOI.INFEASIBLE_OR_UNBOUNDED
        ...
        TotalOperatingCost_MPEC_Firm6=0
        println("Total Operating Cost, Firm 6:", TotalOperatingCost_MPEC_Firm6)
        ...
        Totalrevenue_MPEC_Firm6= 0
        println("Total Revenue, Firm 6:", Totalrevenue_MPEC_Firm6)
        ...
        Total_Investments_Technology_Firm5_EPEC=zeros(7)
        ...
        TotalOperatingCost_demandblock_MPEC_Firm6=zeros(length(set_times))
        ...
        Totalrevenue_demandblock_MPEC_Firm6=zeros(length(set_times))
    ...
    elseif status == MOI.OPTIMAL || (status == MOI.TIME_LIMIT && has_values(m))
        ...
        TotalOperatingCost_MPEC_Firm4=sum(sum(sum((sum(varcost_thermal[e]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==4)
                                                   +sum(varcost_wind[w]*p_G_w_value_MPEC[w,t,s]  for w in set_winds if tech_wind[w,ownership]==4))*γ[s]*Ns_H[t] for s in set_scenarios) for t in set_times))
        ...
        TotalOperatingCost_MPEC_Firm6=sum(sum(sum((sum(varcost_thermal[e]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==6)
                                                   +sum(varcost_wind[w]*p_G_w_value_MPEC[w,t,s]  for w in set_winds if tech_wind[w,ownership]==6))*γ[s]*Ns_H[t] for s in set_scenarios) for t in set_times))
        ...
        Totalrevenue_MPEC_Firm4= sum(sum(sum((sum(Electricity_prices_MPEC[MapG[e][2],t,s]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==4)
                                     +sum(Electricity_prices_MPEC[MapW[w][2],t,s]*p_G_w_value_MPEC[w,t,s] for w in set_winds if tech_wind[w,ownership]==4)
                                     +sum(Spinning_prices_MPEC[t,s]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==4))*γ[s]*Ns_H[t] for s in set_scenarios) for t in set_times))
        ...
        Totalrevenue_MPEC_Firm6= sum(sum(sum((sum(Electricity_prices_MPEC[MapG[e][2],t,s]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==6)
                                     +sum(Electricity_prices_MPEC[MapW[w][2],t,s]*p_G_w_value_MPEC[w,t,s] for w in set_winds if tech_wind[w,ownership]==6)
                                     +sum(Spinning_prices_MPEC[t,s]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==6))*γ[s]*Ns_H[t] for s in set_scenarios) for t in set_times))
        ...
        Total_Investments_Technology_Firm5_EPEC[i]=sum(x_e_value_MPEC[e] for e in set_thermalgenerators if tech_thermal[e,ownership]==5 && tech_thermal[e,Technology]== i)
        end
        ...
        Total_Investments_Technology_Firm5_EPEC[j]=sum(x_w_value_MPEC[w] for w in set_winds if tech_wind[w,ownership]==5 && tech_wind[w,Technology]== i)
        global j=j+1
        end
        ...
        for t in set_times
        TotalOperatingCost_demandblock_MPEC_Firm4[t]=sum(sum((sum(varcost_thermal[e]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==4)
        ...
        for t in set_times
        TotalOperatingCost_demandblock_MPEC_Firm6[t]=sum(sum((sum(varcost_thermal[e]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==6)
                                                   +sum(varcost_wind[w]*p_G_w_value_MPEC[w,t,s]  for w in set_winds if tech_wind[w,ownership]==6))*γ[s]*Ns_H[t] for s in set_scenarios))
        end
        ...
        for t in set_times
        Totalrevenue_demandblock_MPEC_Firm4[t]= sum(sum((sum(Electricity_prices_MPEC[MapG[e][2],t,s]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==4)
                                             +sum(Electricity_prices_MPEC[MapW[w][2],t,s]*p_G_w_value_MPEC[w,t,s] for w in set_winds if tech_wind[w,ownership]==4)
        ...
        for t in set_times
        Totalrevenue_demandblock_MPEC_Firm6[t]= sum(sum((sum(Electricity_prices_MPEC[MapG[e][2],t,s]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==6)
                                             +sum(Electricity_prices_MPEC[MapW[w][2],t,s]*p_G_w_value_MPEC[w,t,s] for w in set_winds if tech_wind[w,ownership]==6)
                                             +sum(Spinning_prices_MPEC[t,s]*p_G_e_value_MPEC[e,t,s] for e in set_thermalgenerators if tech_thermal[e,ownership]==6))*γ[s]*Ns_H[t] for s in set_scenarios))
        end
