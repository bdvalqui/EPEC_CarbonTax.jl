function Solver_EPEC_Diag_StartingPoints_Nfirms_PJM(Number_Firms,start_point,scenario,scenario_tax,scenario_ownership,Trans_Cap,optimizer::Type{<:AbstractOptimizer},resultfile::String="",folder::String="")

global iterations=1:8
...
global Total_Investments_Technology_Firms_EPEC_par_iter=zeros(Number_of_investment_options_thermal_renewables,length(iterations),Number_Firms)
global Total_Investments_Technology_Firms_EPEC_par=zeros(Number_of_investment_options_thermal_renewables,Number_Firms)

global Total_Investments_Technology_Firms_EPEC_par_iter_dummy1=zeros(Number_of_investment_options_thermal_renewables,length(iterations)-1,Number_Firms)
global Total_Investments_Technology_Firms_EPEC_par_iter_dummy2=zeros(Number_of_investment_options_thermal_renewables,length(iterations)-1,Number_Firms)

global Total_Investments_Technology_AllFirms_EPEC_par_iter_diff=zeros(length(iterations)-1,Number_Firms)

global Profits_Objective_Function_par=zeros(Number_Firms)

global Number_Total_Firms=Number_Firms+1

#Deterministic Equivalent------------------------------------------------------#
println("Loading inputs...")
println("Solving Central Planner to get initial conditions...")

(syscost_det,x_w_value,x_e_value,set_opt_winds_numbertechnologies,set_opt_thermalgenerators_numbertechnologies,Total_Investments_Technology_CP)=Solver_CentralPlanner_scenario_PJM(scenario_tax,Trans_Cap,optimizer,"results/CentralPlanner_Solution.txt")
...
#Run EPEC (A set of interrelated MPECs)
(set_thermalgenerators_options,set_wind_options,set_thermalgenerators_existingunits,set_winds_existingunits,set_thermalgenerators,set_winds,set_demands,set_nodes,set_nodes_ref,set_nodes_noref,set_scenarios,set_times,P,V,thermal_ownership_options,wind_ownership_options,x_w_p,x_e_p,Leader, p_D,D,max_demand,Υ_SR,γ,Τ,p_lambda_upper,wind,Ns_H,n_link,links,links_rev,F_max_dict,B_dict,MapG,MapD,MapW,tech_thermal,tech_wind,capacity_per_unit,var_om,invcost,maxBuilds,ownership,capacity_existingunits,fixedcost,EmissionsRate,HeatRate,fuelprice,life,RamUP,Technology,WACC,varcost_thermal,varcost_wind,CRF_thermal,CRF_wind)=loadinputs_startingpoints_Nfirms_PJM(Number_Total_Firms,x_w_value,x_e_value,start_point,scenario_tax,scenario_ownership,Trans_Cap,"data")
...
global Profits_Objective_Function_par[Leader]=profit_det_MPEC

global Cummulative_Emissions_EPEC=zeros(Number_of_technologies_thermal_renewables,2)
...
global Total_Emissions_Technology_Existing_EPEC_par=zeros(Number_of_technologies_thermal_renewables,length(set_times))
global Total_Emissions_Technology_Candidate_EPEC_par=zeros(Number_of_technologies_thermal_renewables,length(set_times))
...
global revenue_cost_EPEC=zeros(Number_Total_Firms*2)


for firm in 1:Number_Total_Firms
global revenue_cost_EPEC[2*firm-1]=Totalrevenue_MPEC_Firms[firm]
global revenue_cost_EPEC[2*firm]=TotalOperatingCost_MPEC_Firms[firm]
end
...
global df_Total_Investments_Technology_Firms_EPEC=DataFrame(Total_Investments_Technology_Firms_EPEC[:,:],:auto)

global Total_Revenue_Cost_demandblock_EPEC=zeros(length(set_times),Number_Total_Firms*2)

for firm in 1:Number_Total_Firms
for t in set_times
global Total_Revenue_Cost_demandblock_EPEC[t,2*firm-1]=Totalrevenue_demandblock_MPEC_Firms[t,firm]
global Total_Revenue_Cost_demandblock_EPEC[t,2*firm]=TotalOperatingCost_demandblock_MPEC_Firms[t,firm]
end
end
...
global Total_Investments_Technology_Firms_EPEC_par[:,firm]=Total_Investments_Technology_Firms_EPEC[:,firm]
global Total_Investments_Technology_Firms_EPEC_par_iter[:,iter,firm]=Total_Investments_Technology_Firms_EPEC[:,firm]
end
...
for firm in 1:Number_Firms
global Total_Investments_Technology_Firms_EPEC_par_iter_dummy1[:,1:iter-1,firm]=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 2:iter,firm)
global Total_Investments_Technology_Firms_EPEC_par_iter_dummy2[:,1:iter-1,firm]=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 1:iter-1,firm)
end

for firm in 1:Number_Firms
global Total_Investments_Technology_AllFirms_EPEC_par_iter_diff[:,firm]=sum(abs.(Total_Investments_Technology_Firms_EPEC_par_iter_dummy1[:,:,firm]-Total_Investments_Technology_Firms_EPEC_par_iter_dummy2[:,:,firm]),dims=1)
end

global Total_Investments_Technology_Firms_EPEC_par_iter_diff=sum(Total_Investments_Technology_AllFirms_EPEC_par_iter_diff,dims=2)
...
#=
global Total_Investments_Technology_Firm1_EPEC_par_iter_dummy1=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 2:iter,1)
global Total_Investments_Technology_Firm1_EPEC_par_iter_dummy2=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 1:iter-1,1)

global Total_Investments_Technology_Firm2_EPEC_par_iter_dummy1=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 2:iter,2)
global Total_Investments_Technology_Firm2_EPEC_par_iter_dummy2=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 1:iter-1,2)
=#

#global Total_Investments_Technology_Firms_EPEC_par_iter_dummy=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 2:iter,1)

#=
global Total_Investments_Technology_Firms_EPEC_par_iter_dummy1[:,1:iter-1,2]=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 2:iter,2)
global Total_Investments_Technology_Firms_EPEC_par_iter_dummy2[:,1:iter-1,2]=view(Total_Investments_Technology_Firms_EPEC_par_iter, :, 1:iter-1,2)
=#
...
#global Total_Investments_Technology_AllFirms_EPEC_par_iter_diff[:,1]=sum(abs.(Total_Investments_Technology_Firms_EPEC_par_iter_dummy1[:,:,1]-Total_Investments_Technology_Firms_EPEC_par_iter_dummy2[:,:,1]),dims=1)
#global Total_Investments_Technology_AllFirms_EPEC_par_iter_diff[:,2]=sum(abs.(Total_Investments_Technology_Firms_EPEC_par_iter_dummy1[:,:,2]-Total_Investments_Technology_Firms_EPEC_par_iter_dummy2[:,:,2]),dims=1)
...
#global Nash_Equilibrium_Indicator=0
...
println("Nash_Equilibrium_Indicator of iteration $iter: $Nash_Equilibrium_Indicator")
...
#Save Results for debugging. These lines should be deleted in the next version


#=
df_Total_Investments_Technology_Firm1_EPEC_par_iter_dummy1= DataFrame(Total_Investments_Technology_Firm1_EPEC_par_iter_dummy1,:auto)

CSV.write(folder *"/Total_Investments_Technology_Firm1_EPEC_par_iter_dummy1$Τ.csv", df_Total_Investments_Technology_Firm1_EPEC_par_iter_dummy1)

df_Total_Investments_Technology_Firm1_EPEC_par_iter_dummy2= DataFrame(Total_Investments_Technology_Firm1_EPEC_par_iter_dummy2,:auto)

CSV.write(folder *"/Total_Investments_Technology_Firm1_EPEC_par_iter_dummy2$Τ.csv", df_Total_Investments_Technology_Firm1_EPEC_par_iter_dummy2)

df_Total_Investments_Technology_Firm2_EPEC_par_iter_dummy1= DataFrame(Total_Investments_Technology_Firm2_EPEC_par_iter_dummy1,:auto)

CSV.write(folder *"/Total_Investments_Technology_Firm2_EPEC_par_iter_dummy1$Τ.csv", df_Total_Investments_Technology_Firm2_EPEC_par_iter_dummy1)

df_Total_Investments_Technology_Firm2_EPEC_par_iter_dummy2= DataFrame(Total_Investments_Technology_Firm2_EPEC_par_iter_dummy2,:auto)

CSV.write(folder *"/Total_Investments_Technology_Firm2_EPEC_par_iter_dummy2$Τ.csv", df_Total_Investments_Technology_Firm2_EPEC_par_iter_dummy2)



df_Total_Investments_Technology_Firm1_EPEC_par_iter_diff= DataFrame(Total_Investments_Technology_Firm1_EPEC_par_iter_diff',:auto)

CSV.write(folder *"/Total_Investments_Technology_Firm1_EPEC_par_iter_diff$Τ.csv", df_Total_Investments_Technology_Firm1_EPEC_par_iter_diff)

df_Total_Investments_Technology_Firm2_EPEC_par_iter_diff= DataFrame(Total_Investments_Technology_Firm2_EPEC_par_iter_diff',:auto)

CSV.write(folder *"/Total_Investments_Technology_Firm2_EPEC_par_iter_diff$Τ.csv", df_Total_Investments_Technology_Firm2_EPEC_par_iter_diff)



df_Total_Investments_Technology_Firms_EPEC_par_iter_Firm1 =  DataFrame(Total_Investments_Technology_Firms_EPEC_par_iter[:,:,1], :auto)

df_Total_Investments_Technology_Firms_EPEC_par_iter_Firm2 =  DataFrame(Total_Investments_Technology_Firms_EPEC_par_iter[:,:,2], :auto)

CSV.write(folder *"/Total_Investments_Technology_Firms_EPEC_par_iter_Firm1$Τ.csv", df_Total_Investments_Technology_Firms_EPEC_par_iter_Firm1)

CSV.write(folder *"/Total_Investments_Technology_Firms_EPEC_par_iter_Firm2$Τ.csv", df_Total_Investments_Technology_Firms_EPEC_par_iter_Firm2)


df_Total_Investments_Technology_Firms_EPEC_par_iter_dummy1_Firm1 =  DataFrame(Total_Investments_Technology_Firms_EPEC_par_iter_dummy1[:,:,1], :auto)

df_Total_Investments_Technology_Firms_EPEC_par_iter_dummy2_Firm1 =  DataFrame(Total_Investments_Technology_Firms_EPEC_par_iter_dummy2[:,:,1], :auto)

df_Total_Investments_Technology_Firms_EPEC_par_iter_dummy1_Firm2 =  DataFrame(Total_Investments_Technology_Firms_EPEC_par_iter_dummy1[:,:,2], :auto)

df_Total_Investments_Technology_Firms_EPEC_par_iter_dummy2_Firm2 =  DataFrame(Total_Investments_Technology_Firms_EPEC_par_iter_dummy2[:,:,2], :auto)


CSV.write(folder *"/Total_Investments_Technology_Firms_EPEC_par_iter_dummy1_Firm1$Τ.csv", df_Total_Investments_Technology_Firms_EPEC_par_iter_dummy1_Firm1)

CSV.write(folder *"/Total_Investments_Technology_Firms_EPEC_par_iter_dummy2_Firm1$Τ.csv", df_Total_Investments_Technology_Firms_EPEC_par_iter_dummy2_Firm1)

CSV.write(folder *"/Total_Investments_Technology_Firms_EPEC_par_iter_dummy1_Firm2$Τ.csv", df_Total_Investments_Technology_Firms_EPEC_par_iter_dummy1_Firm2)

CSV.write(folder *"/Total_Investments_Technology_Firms_EPEC_par_iter_dummy2_Firm2$Τ.csv", df_Total_Investments_Technology_Firms_EPEC_par_iter_dummy2_Firm2)

df_Total_Investments_Technology_AllFirms_EPEC_par_iter_diff= DataFrame(Total_Investments_Technology_AllFirms_EPEC_par_iter_diff,:auto)

CSV.write(folder *"/Total_Investments_Technology_AllFirms_EPEC_par_iter_diff$Τ.csv", df_Total_Investments_Technology_AllFirms_EPEC_par_iter_diff)


df_Total_Investments_Technology_Firms_EPEC_par_iter_diff= DataFrame(Total_Investments_Technology_Firms_EPEC_par_iter_diff',:auto)

CSV.write(folder *"/Total_Investments_Technology_Firms_EPEC_par_iter_diff$Τ.csv", df_Total_Investments_Technology_Firms_EPEC_par_iter_diff)
=#
...
#I will need to adjust this for N firms!!!
...
resultfile != "" && open(resultfile, truncate = true) do f
    ...
    println(f,"")
    println(f,"Objective Functions: ",Profits_Objective_Function_par)
    println(f,"")
...
return (Profits_Objective_Function_par,Total_Investments_Technology_EPEC_par,Nash_Equilibrium_Indicator_bin)
