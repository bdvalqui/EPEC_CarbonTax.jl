...
@variable(m,Total_Investments_Technology_variable_CP[tech in 1:7])
...
@constraint(m, Total_Investments_Technology_variable_CP[j]==sum(x_w[w] for w in set_opt_winds if tech_wind_opt[w,Technology]== i))
global j=j+1
end

@constraint(m, Total_Investments_Technology_variable_CP[5]==(3.9/100)*(sum(x_w[w] for w in set_opt_winds)+sum(x_e[e] for e in set_opt_thermalgenerators)))

@constraint(m, Total_Investments_Technology_variable_CP[6]==(60.4/100)*(sum(x_w[w] for w in set_opt_winds)+sum(x_e[e] for e in set_opt_thermalgenerators)))

@constraint(m, Total_Investments_Technology_variable_CP[7]==(35.7/100)*(sum(x_w[w] for w in set_opt_winds)+sum(x_e[e] for e in set_opt_thermalgenerators)))
